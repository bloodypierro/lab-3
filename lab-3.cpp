...
int main()
{
    // Пункт 1
    int amount;
    cout << "Количество чисел: "; // исправил опечатку "Количествое"
    cin >> amount;

    int sum = 0, count = 0, maxNeg = -9999999;
    bool found = false;

    cout << "Введите " << amount << " чисел: "; // исправил "числа" на "чисел"
    ...
    if (!found) {
        ...
        cout << "Сумма отрицательных: " << sum << endl;  // добавил пробелы
    ...
    // Пункт 2
    int x;
    cout << "Введите X (|X| < 1000): ";  // добавил подсказку
    cin >> x;

    // Исправил условие проверки
    if (x >= -999 && x <= 999) {  // правильное условие для |X| < 1000
        if (x < 0) x = -x;
        ...
        cout << "Самая большая цифра: " << max << endl;  // исправил опечатку "Сама"
    }
    else {
